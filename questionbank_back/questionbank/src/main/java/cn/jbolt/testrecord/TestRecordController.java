...
import cn.jbolt.apibean.ReportListBean;
...
public class TestRecordController extends Controller {
	...
	public void getMyRecords() {
		...
		reportListBean.setCode(1);
		List<Testrecord> testrecords = aPage.getList();
		List<ReportBean> reportBeans = new ArrayList<>(testrecords.size());
		for (Testrecord testrecord : testrecords) {
			reportBeans.add(createReport(testrecord));
		}
		reportListBean.setReportBeans(reportBeans);
		renderJson(reportListBean);
	}
}
